  ...
  return (
    <AlertDialog>
      <div className="flex flex-col">
        <CreateSubaccountButton user={user} id={params.agencyId} className='w-[200px] self-end m-6'></CreateSubaccountButton>
                ...
                <CommandItem key={subaccount.id} className='h-32 !bg-background my-2 text-primary border-[1px] border-border p-4 rounded-lg hover:!bg-background cursor-pointer transition-all'>
                  <Link href={`/subaccount/${subaccount.id}`} className='flex gap-4 w-full h-full'>
                    <div className="relative w-32">
                      <Image src={subaccount.subAccountLogo} alt="subAccount logo"  className='rounded-md object-contain bg-muted/50 p-4' fill></Image>
                    </div>
                  ...
                  <AlertDialogTrigger asChild>
                    <Button size="sm" variant={'destructive'} className='text-red-600 w-20 hover:bg-red-600 hover:text-white'>Delete</Button>
                  </AlertDialogTrigger>
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      <AlertDialogTitle className='text-left'>Are you absolutely sure</AlertDialogTitle>
                      <AlertDescription className='text-left'>This action cannot be undone. This will delete the subaccount and all data related to the subaccount.</AlertDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter className='flex items-center flex-row w-full'>
